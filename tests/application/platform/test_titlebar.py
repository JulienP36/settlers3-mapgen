...
from s3mapgen.application.platform.titlebar import (
    DWMWA_BORDER_COLOR,
    DWMWA_CAPTION_COLOR,
    DWMWA_TEXT_COLOR,
...
GUI_SOURCE = Path('s3mapgen/application/main_window.py').read_text(encoding='utf-8')
...
def test_non_windows_platform_is_a_safe_noop(monkeypatch):
    monkeypatch.setattr('s3mapgen.application.platform.titlebar.sys.platform', 'linux')
...
def test_separator_is_a_real_client_edge_overlay_not_only_a_dwm_border():
    source = Path('s3mapgen/application/platform/titlebar.py').read_text(encoding='utf-8')
