...
- Au début d’une tâche, lire `PROJECT_WORKFLOW.md`, puis, lorsqu’ils sont
  présents dans la copie source/ZIP, `references/REFERENCE_INDEX.md`,
  `references/SETTLERS3_CURRENT_SNAPSHOT.md` et uniquement la section utile de
  `TODO_MAPGEN.md`.
...
- Avant toute modification de génération, EDM/MAP/SAV, export ou données
  natives, lire `references/REFERENCE_INDEX.md`, puis
  `references/SETTLERS3_PREGEN_READ_FIRST.md` et vérifier les
  hashes protégés de `PROJECT_WORKFLOW.md`. Le reset DEV_2 autorise la
  suppression des anciennes ressources de génération lorsqu'elles sont
  remplacées par un chemin actif clairement identifié.
