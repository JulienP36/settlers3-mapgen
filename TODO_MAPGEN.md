...
- [x] **v1.5 VALIDÉE / STABLE** : candidate `S3_V1_5_V7NOGAP_CORRECTED_UPGRADED_4P_768x768_seed_2026082202` ouverte dans l'éditeur, starts OK, View Map/in-game sans crash, rendu général validé. Le test pratique de construction sur ID127 est différé en micro-map et n'est pas bloquant pour la release.
...
- [x] **États Building Stones variés** : Legacy distribution native-like ; Upgraded distribution variée mais biaisée vers les pierres plus pleines, avec ajustement fin pour atteindre exactement le stock cible. Répartition visuelle `115..127` validée.
- [x] **Building Stone 13 / ID127 vide généré** : comportement natif confirmé. Cible pratique 20 ancres globales sur 1683 ; 0 unité ; exclues du stock exploitable ; jamais dans les clusters bonus de start.
- [x] **ID127 constructible dans le modèle statique** : contrairement aux états actifs, le tas épuisé ne bloque plus la construction ; son ancien footprint 7 cellules est remis en `accessibility=0` avant validation/export.
...
- [x] **Récifs Upgraded protégés des bords** : marge minimale de 2 cellules ; validator `REEF_EDGE_MARGIN`.
...
- [x] **Bonus de départ Upgraded validés visuellement** : clusters centrés sur la bordure du territoire initial (~HEX34). Forêt : 41 adultes + 21 SmallTree84/joueur. Stones : 8 ancres/joueur, 84 unités/joueur, 9..12 unités/ancre. Mini-marais inchangé.
- [x] GUI/CLI en **v1.5** (`gui_v15.py`, runtime final `generator_v15.py`, exports `MapGenV1_5`).
- [x] Contrôle objets v1.5 : quotas exacts ; 20 ID127 ; 13 états 115..127 ; aucun footprint ID127 statiquement bloquant ; clusters bonus validés.
- [x] **Contrôle ressources corrigé `S3_V1_5_V7NOGAP_CORRECTED_UPGRADED_4P_768x768_seed_2026082202` validé** : formes de minerais exactement conformes au style long-play v7. L'ancien candidat `seed_2026082201` est invalidé et ne doit jamais servir de référence.
- [x] **Contrôle final éditeur/View Map/in-game PASS** : ouverture sans erreur, starts valides, aucun crash, rendu général OK. v1.5 promue stable.
- [ ] Micro-map de régression facultative : poser explicitement un ID127 près d'une zone constructible et confirmer rapidement côté éditeur que le bâtiment peut recouvrir l'ancien footprint. Non bloquant pour v1.5.
...
- [ ] **Réaliste** : distribution écologique plus crédible sans changer la macro-géographie. Pistes : arbres/plantes favorisés près de l'eau, champignons favorisés près des marais/sols humides, végétation modulée par biome/relief/humidité, avec priorité à la constructibilité, aux ressources et au gameplay.
