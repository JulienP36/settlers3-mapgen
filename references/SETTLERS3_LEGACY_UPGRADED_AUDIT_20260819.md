...
Date: 2026-08-19
Status: **canonical mode-separation reference — v1.5 candidate baseline**
...
- Building Stone active states `115..126` use the confirmed blocking 7-cell footprint.
- Building Stone 13 / `127` is the exhausted visual state: **0 stock and buildable**. It remains an object/anchor for density statistics, but its former 7-cell footprint must be released (`accessibility=0`) so construction is possible over it.
...
- Native exhausted state `127` is also generated: 768 references show 18/22/21 exhausted anchors, so v1.5 uses a practical target of **20 global ID127 anchors out of 1683**.
...
- Current empirical native family shares retained as calibration source:
  - Coal 50.186%
  - Iron 21.564%
  - Gold 14.417%
  - Gems 5.446%
...
- Practical p99 fit: `river_p99_target(side) ~= 0.0245 * side + 34.7`.
- Treat this as a practical target, **not a strict absolute maximum**; rare native tails may be longer.
...
- Current Upgraded target: about **130% of the correct total native tree volume**.
...
- Active functional states `115..126`, with `remaining = 127 - object_id`.
- State `127` is generated as an exhausted/empty visual remnant, contributes zero resource units and is buildable.
- Global 768 target: **1683 total anchors**, including **20 exhausted ID127**; exact stock target is carried only by active states.
- Improved stock profile retained; spatial behavior mixes small clusters/fields with scattered anchors.
- Global Upgraded states remain varied while being biased toward fuller stones; exact total stock is enforced by small per-anchor corrections.
- Start bonus remains Upgraded-only and outside global quota; start clusters never contain ID127.
...
- Initial territory is modeled with a practical HEX radius of about **34** cells. Bonus cluster centers sit on this boundary so the territory border passes through each cluster.
- **Bonus forest per player:** approximately one ordinary Upgraded forest-cluster volume, currently **41 adult trees + 21 SmallTree84**.
- **Bonus Building Stone cluster per player:** **8 anchors**, matching the ordinary Upgraded cluster scale. Stones are deliberately well filled but still varied, with an exact target of **84 remaining stone units/player** (individual anchors 9..12 units).
...
- **Mountain realism**: possible future explicit mountain/relief modifier rather than silently diverging Upgraded Snow/mountain rules.
- **Réaliste**: ecological placement modifier. Candidate ideas include more trees/plants near water, mushrooms favored around swamp/humid ground, and vegetation probabilities conditioned by biome/relief/humidity, while preserving construction space, resources and gameplay.
...
- Validate Building Stone state diversity and ID127 constructibility visually/in-game in both Legacy and Upgraded.
