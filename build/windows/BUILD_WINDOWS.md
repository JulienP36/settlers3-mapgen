...
The script installs runtime/build dependencies, derives archive and Windows metadata from the runtime version, creates the folder, runs the packaged executable's startup/resource self-test and writes the ZIP plus its SHA-256 file under `artifacts/`. Git attributes force the protected text resources to retain their repository LF bytes on Windows; the workflow verifies all five v1.5 baseline hashes before building, and the packaged self-test imports the normal GUI runtime before rechecking the three protected resources actually bundled.
