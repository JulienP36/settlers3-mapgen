...
- `v1.4 candidate` — dark/light UI, persistent settings, overlays, improved progress/navigation, parallelogram visualization, native-sized start-territory outlines, crisp player labels, dark combobox fixes and click-to-position sliders.

The v1.4 line is still a **candidate** until the latest visualization/interface corrections are revalidated by the user. It does not introduce new Legacy/Upgraded generation rules over the validated v1.3.2 engine.

Known issue tracked separately before broader generation work: in one game-launch control, starting supplies left on `Défaut` caused an error/crash while explicit `Low / Medium / High` presets did not.
...
5. create an annotated version tag once the candidate is explicitly promoted;
6. push branch + tags;
