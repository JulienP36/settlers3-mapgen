...
class App(tk.Tk):
    def __init__(self):
        super().__init__();self.title('Settlers III MapGen v1.3');self.geometry('1380x860');self.minsize(1080,720)
    ...
    def export(self):
        ...
        base=f"S3_{st.metadata.get('archetype','Imported')}_{st.metadata.get('mode','Map')}_{len(st.starts) or st.metadata.get('players',0)}P_{side}x{side}_seed_{st.metadata.get('seed','import')}_MapGenV1_3".replace(' ','')
        made=[]
        if side==768:
