"""Provisional player-start bridge for procedural Upgraded maps."""
...
def place_starts(state, players: int, rng: np.random.Generator, *, technical_clear: int = 14) -> np.ndarray:
    ...
    The Upgraded pipeline currently calls this after its copied terrain pass,
    solely to provide MAP/EDM coordinates.  It does not create start resources,
    ...
    state.metadata["starts_placed_early"] = False
    state.metadata["starts_placed_provisionally"] = True
