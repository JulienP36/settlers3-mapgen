...
def validate(state, profile: dict) -> list[ValidationResult]:
    ...
    border[[0, -1], :] = True
    border[:, [0, -1]] = True
    add("UPGRADED_FISH_TARGET", int(fish.sum()) == int(profile["fish"]["target_cells"]), f"{int(fish.sum())}/{int(profile['fish']['target_cells'])}")
    ...
    add("UPGRADED_ADULT_TREES", adult == int(profile["trees"]["adult_global_target"]), f"{adult}/{int(profile['trees']['adult_global_target'])}")
    add("UPGRADED_SMALL_TREES", small == int(profile["trees"].get("small_tree_target", 0)), f"{small}/{int(profile['trees'].get('small_tree_target', 0))}")
    add("UPGRADED_PALMS", palms == int(profile["trees"].get("palm_target", 0)), f"{palms}/{int(profile['trees'].get('palm_target', 0))}")
    ...
    target_anchors = int(profile["building_stones"]["global_anchor_target"])
    stock = int(np.sum(int(profile["building_stones"]["exhausted_id"]) - objects[stone]))
    target_stock = int(profile["building_stones"]["global_stock_target"])
